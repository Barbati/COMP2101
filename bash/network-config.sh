...
dfRouteName=$(cat /etc/hosts | grep -i 192.168.139.2 | awk '{print $2}')
netNumb=$(cat /etc/networks | grep -i link-local | awk '{print $2}')
netName=$(cat /etc/networks | grep -i link-local | awk '{print $1}')
...
Router Address  : $dfRoute
Router Hostname : $dfRouteName
